...
@dataclass
class RandomConsecutiveMultiviewCameraDataModuleConfig(RandomCameraDataModuleConfig):
    relative_radius: bool = True
    n_view: int = 1
    zoom_range: Tuple[float, float] = (1.0, 1.0)
...
class RandomConsecutiveMultiviewCameraIterableDataset(RandomCameraIterableDataset):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.zoom_range = self.cfg.zoom_range
        self.random_seed = 0
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        self.num_of_calls += 1
        if self.num_of_calls % self.cfg.n_times == 0:
            self.random_seed += 1
        assert (
            self.batch_size % self.cfg.n_view == 0
        ), f"batch_size ({self.batch_size}) must be dividable by n_view ({self.cfg.n_view})!"
...
class ReplicatedIterableDataset(IterableDataset):
    def __init__(self, original_dataset, n):
    ...
    @property
    def collate(self):
        return self.original_dataset.collate if hasattr(self.original_dataset, 'collate') else None

@register("mvdream-consecutive-random-multiview-camera-datamodule")
class RandomConsecutiveMultiviewCameraDataModule(pl.LightningDataModule):
    cfg: RandomConsecutiveMultiviewCameraDataModuleConfig

    def __init__(self, cfg: Optional[Union[dict, DictConfig]] = None) -> None:
        super().__init__()
        self.cfg = parse_structured(RandomConsecutiveMultiviewCameraDataModuleConfig, cfg)
